    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                logger.warning(
                    "Network error on attempt %d, retrying: %s", attempt + 1, e
                )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_message = (
        "Generate a short, friendly welcome message introducing yourself as Alfred."
    )
    ...
    with open(alfred_sticker_path, "rb") as sticker_file:
        await send_with_retry(
            lambda: context.bot.send_sticker(chat_id=chat_id, sticker=sticker_file)
        )

    await send_with_retry(
        lambda: context.bot.send_message(
            chat_id=chat_id, text=agent.run(welcome_message)
        )
    )
...
async def text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading_sticker = (
        "CAACAgIAAxkBAAIBFmmk8vIL3M3TJ2oREadRc925e6BFAAK0IwACmEspSN65vs0qW-TZOgQ"
    )

    telegram_message_logger(update)

    sticker_message = await send_with_retry(
        lambda: context.bot.send_sticker(chat_id=chat_id, sticker=loading_sticker)
    )
    response = await asyncio.get_event_loop().run_in_executor(
        None, lambda: agent.run(user_message)
    )
    await context.bot.delete_message(
        chat_id=chat_id, message_id=sticker_message.message_id
    )
    await send_with_retry(
        lambda: context.bot.send_message(chat_id=chat_id, text=response)
    )
